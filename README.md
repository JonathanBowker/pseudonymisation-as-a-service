...
Pseudonymisation uses forms of encryption to translate identifiable personal data points to unique artificial identifiers - _Pseudonyms_. Pseudonymisation techniques aims to de-couple individuals from their personal data held by data custodians - this renders the data ‘pseudonymous’ for specific use cases. Beyond the context of a specific use case the person can still be re-identified through protected procedures and controls governed by a data custodian - _**e.g. unit within an organisation, personal information managemet system, interconnected network, or third party provider**_. 

Pseudonymised data can still be traced to it's underlying data subject but it requires external information to do so. Within pseudonymised data, the plain text data points still exist but only within a physically and logically separated system joined by securely controlled linking functions.
